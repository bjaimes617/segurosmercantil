                            ...
                            <table id="usuarios_table" class="table table-bordered table-striped text-center">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        {!!Form::open (['route' => ['users.destroy',$user->id], 'method' => 'delete', 'id' => 'delete_users'.$user->id]) !!}                                         
                                        @permission('restore.users')
                                        <a href="{{route('users.restore',$user->id)}}" data-toggle="tooltip" data-placement="top" title="Restaurar Usuario"><i class="fas fa-recycle"></i></a>
                                        @endpermission
                                        @permission('delete.users')
                                        <a href="#" onclick="if(!confirm('¿Desea eliminar el usuario definitivamente?'))
                                        { return false; } else {$('#delete_users'+'{!!$user->id!!}').submit(); }" data-toggle="tooltip" data-placement="top" title="Eliminar Usuario"><i class="fas fa-user-times"></i></a>
                                        {!!Form::close() !!} 
                                        @endpermission
                                        @endpermission
                                        </td>
